<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Gerencie os ingredientes dos produtos</p>
            </div>
            <button type="button"
                    @click="$dispatch('open-ingredient-modal', { mode: 'create' })"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" x-model="filters.search"
                           @keydown.enter="fetchIngredients()"
                ...
                <div class="flex items-end gap-2">
                    <button type="button" @click="fetchIngredients()" :disabled="loading"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        <template x-for="ingredient in ingredients" :key="ingredient.id">
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                                <td class="px-6 py-2 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100" x-text="ingredient.name"></div>
                                </td>
                                <td class="px-6 py-2">
                                    <div class="text-sm text-gray-500 dark:text-gray-400" x-text="ingredient.description || '-'"></div>
                                </td>
                                <td class="px-6 py-2 whitespace-nowrap text-center">
                                    <span x-show="ingredient.is_active" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300">
                                        Ativo
                                    </span>
                                    <span x-show="!ingredient.is_active" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300">
                                        Inativo
                                    </span>
                                </td>
                                <td class="px-6 py-2 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex items-center justify-center gap-1">
                                        <button type="button"
                                                @click="$dispatch('open-ingredient-modal', { mode: 'edit', ingredient: ingredient })"
                                        ...
                                        <button type="button"
                                                @click="$dispatch('open-history-modal', { ingredient: ingredient })"
                                        ...
                                        <button type="button"
                                                ...
                                                :class="ingredient.is_active ? 'text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/50' : 'text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300 hover:bg-green-50 dark:hover:bg-green-900/50'"
                                                class="p-2 rounded-lg transition-colors duration-150"
                                                :title="ingredient.is_active ? 'Inativar' : 'Ativar'">
                                            <svg x-show="ingredient.is_active" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr x-show="ingredients.length === 0 && !loading">
                            <td colspan="4" class="text-center">
                                <div class="py-12">
                                    <svg class="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/>
                                    </svg>
                                    <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">Nenhum ingrediente encontrado</h3>
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tente ajustar os filtros ou cadastre um novo ingrediente.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <!-- Pagination -->
            <div x-show="pagination.total > 0" class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Mostrando <span x-text="((pagination.current_page - 1) * pagination.per_page) + 1"></span>
                    até <span x-text="Math.min(pagination.current_page * pagination.per_page, pagination.total)"></span>
                    de <span x-text="pagination.total"></span> ingredientes
                </div>
    ...
    <div x-data="{
            ...
            errors: {},
            get title() {
                return this.mode === 'create' ? 'Novo Ingrediente' : 'Editar Ingrediente';
            },
            get buttonText() {
                return this.mode === 'create' ? 'Criar Ingrediente' : 'Salvar Alterações';
            },
            resetForm() {
                this.form = {
                    uuid: null,
            ...
            async save() {
                ...
                try {
                    ...
                    this.open = false;
                    this.resetForm();
                    window.dispatchEvent(new CustomEvent('ingredient-saved', { detail: data }));
                } catch (error) {
         ...
         @open-ingredient-modal.window="
            open = true;
            mode = $event.detail.mode;
            if (mode === 'edit' && $event.detail.ingredient) {
                form = {
                    uuid: $event.detail.ingredient.uuid,
                    name: $event.detail.ingredient.name,
                    description: $event.detail.ingredient.description || '',
                    is_active: $event.detail.ingredient.is_active
                };
            } else {
                resetForm();
            }
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" x-model="form.name"
                                   placeholder="Ex: Queijo, Bacon, Alface..."
    ...
    <div x-data="historyModal()"
         @open-history-modal.window="openModal($event.detail.ingredient)"
         x-show="open"
         x-cloak
         class="fixed inset-0 z-50 overflow-y-auto"
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                <span x-text="ingredientName"></span>
        ...
        function historyModal() {
            return {
                ...
                openModal(ingredient) {
                    this.ingredientUuid = ingredient.uuid;
                ...
                async loadHistory(page) {
                    this.loading = true;
                    try {
                        const response = await fetch('/ingredients/' + this.ingredientUuid + '/history?page=' + page + '&per_page=' + this.pagination.per_page, {
                            method: 'GET',
                            headers: {
                                'Accept': 'application/json'
                            }
        ...
        function ingredientManager() {
            return {
                ...
                init() {
                    ...
                    window.addEventListener('ingredient-saved', (event) => {
                ...
                async fetchIngredients(page = 1) {
                    this.loading = true;
                    try {
                        const response = await fetch('{{ route('ingredients.filter') }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                ...
                async toggleStatus(ingredient) {
                    try {
                        const response = await fetch('/ingredients/' + ingredient.uuid + '/toggle-status', {
                            method: 'PATCH',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
